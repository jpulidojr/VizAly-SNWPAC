...
namespace lossywave
{
	...
	size_t lossywave::decompress(void *data, void *&output)
	{
		...
		size_t decode_size = decode(data, output);
	...
	template <typename T>
	size_t lossywave::decode(T * in, void *& out)
	{
		size_t total = params[4] * params[5] * params[6];
		int cmpBytes = 0;

		//static cast out to type T here
		//
		...
		if (params[2] >= 118)
		{
			...
			if (oval_sz == 8) // Type DOUBLE //TODO: This means no requantization
			{
				char conc[8];
				T dbl_val;
			...
			else if (oval_sz == 4) // Type FLOAT input, may be float/double output
			{
				...
				if (params[10] == 8) // Requantize ints to floats
				{
					...
					for (size_t sz = 0; sz < total; sz++)
					{
						...
						out[sz] = (T)dbl_val * mod_bits; // INT SUPPORT
					}
				}
				else if (params[10] == 4)
				{
